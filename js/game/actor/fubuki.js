class Fubuki extends Actor {
    ...
    draw = (game, cx) => {
        ...
        const offset = new Vector2(24, 16);
        const xSize = 48;
        const spd = this.animation === 'chant' ? 16 : 1;
        const frame = this.animation === 'chant' ? 4 : 1;
        cx.drawImage(game.assets.images[`sp_fubuki_${this.animation}`],
            (Math.floor(this.animationFrame / spd) % frame) * xSize, 0, 64, 48,
            -offset.x, -offset.y, 64, 48);
        cx.restore();
    }
}