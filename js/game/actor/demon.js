class Demon extends Actor {
    ...
    chargePhase = game => {
        ...
        game.scene.particles.smoke_black(this.pos.plus(new Vector2(this.size.x * .5, 64)), new Vector2(random() * 8 - 4, -1), 1);
    ...
    laserPhase = game => {
        for (let i = 0; i < 3; i++) {
            game.scene.particles.smoke_black(new Vector2(this.pos.x + random() * this.size.x, 22 * 16), new Vector2(random() - .5, -random() * 2), 1);
        }
    ...
    update = game => {
        ...
        for (let i = 0; i < 8; i++) {
            game.scene.particles.smoke_pink(CollisionBox.center(this).plus(new Vector2(random() * 48 - 24, random() * 64 - 32)), new Vector2(random() - .5, random() * -2), 0);
        }
...
class DemonHand extends Actor {
    ...
    update = game => {
        ...
        for (let i = 0; i < 2; i++) {
            game.scene.particles.smoke_pink(CollisionBox.center(this).plus(new Vector2(random() * 16 - 8 + 16 * (this.dir ? 1 : -1), random() * 32 - 16)), new Vector2(random() - .5 + .5 * (this.dir ? 1 : -1), random() * -2), 0);
        }
...
class ShirakenHelper extends Actor {
    ...
    update = game => {
        ...
        if (this.demonHand || this.weakBuffer) {
            if (random() > .95) this.shakeBuffer = 2;
            for (let i = 0; i < 2; i++) {
